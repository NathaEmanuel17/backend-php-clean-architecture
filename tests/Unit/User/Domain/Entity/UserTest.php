...
final class UserTest extends TestCase
{
    public function testShouldCreateUser(): void
    {
        ...
        $passwordHash = PasswordHash::fromString(password_hash('StrongPassword123!', PASSWORD_ARGON2ID));

        $user = User::create($id, $name, $email, $passwordHash);

        self::assertSame($id, $user->id());
        self::assertSame($name, $user->name());
        self::assertSame($email, $user->email());
        self::assertSame($passwordHash, $user->passwordHash());
    }
    ...
    public function testShouldChangeUserPasswordHash(): void
    {
        ...
        $newPasswordHash = PasswordHash::fromString(
            password_hash('AnotherStrongPassword123!', PASSWORD_ARGON2ID)
        );

        $user->changePasswordHash($newPasswordHash);

        self::assertSame($newPasswordHash, $user->passwordHash());
    }
}
